...
export function transitionAsync(durationInMilliseconds: number, easeCallback: (t: number) => any, cancellationToken?: CancellationToken) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (cancellationToken) {
            ...
            function stepWithCancellationToken(timestamp: DOMHighResTimeStamp) {
                ...
                if (cancellationToken.isCancellationRequested)
                    ...
                    var progress = Math.min((timestamp - start) / durationInMilliseconds, 1);

                    easeCallback(progress);

                    if (progress < 1)
                        requestAnimationFrame(stepWithCancellationToken);
            ...
            function stepWithoutCancellationToken(timestamp: DOMHighResTimeStamp) {
                ...
                if (progress < 1)
                    requestAnimationFrame(stepWithoutCancellationToken);
...
export interface DOMAnimator {
    ...
    insertBeforeAsync(parentElement: HTMLElement, newChild: HTMLElement, referenceNode: Node): Promise<void>;
...
export var ease = bezier(0.25, 0.1, 0.25, 1.0); // default
export var easeIn = bezier(0.42, 0.0, 1.0, 1.0);
export var easeInOut = bezier(0.42, 0.0, 0.58, 1.0);
export var easeOut = bezier(0.0, 0.0, 0.58, 1.0);